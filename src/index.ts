...
import { ZhihuClient, ZhihuCollectionFolder } from './zhihu.js';
import { Summarizer, SummaryNote } from './summarizer.js';
...
/** 缓存：收藏夹数据 */
let cachedFolders: ZhihuCollectionFolder[] | null = null;
...
class CollectionSummaryServer {
      ...
      async () => ({
        tools: [
          ...
          },
          {
            ...
            description:
              '总结已获取的收藏内容，生成结构化笔记（需要先调用 fetch_all_zhihu_collections）',
            inputSchema: {
              type: 'object',
              properties: {
                folderIndex: {
            ...
            inputSchema: {
              ...
              properties: {
                outputDir: {
                  type: 'string',
                  description:
                    '输出目录（默认：~/Documents/CollectionNotes）',
                },
              },
              required: [],
            },
      ...
      async (request) => {
        switch (request.params.name) {
          ...
          case 'fetch_zhihu_collection_contents':
            return this.handleFetchCollectionContents(
              request.params.arguments
            );
          case 'fetch_all_zhihu_collections':
            return this.handleFetchAllCollections(
              request.params.arguments
            );
   ...
   * 处理：获取收藏夹列表
   */
  private async handleFetchCollections(args: any) {
   ...
   * 处理：获取收藏夹内容
   */
  private async handleFetchCollectionContents(args: any) {
    ...
    try {
      const items = await zhihuClient.getCollectionContents(
        collectionId,
        limit
      );
   ...
   * 处理：一键获取所有收藏
   */
  private async handleFetchAllCollections(args: any) {
    const userId = args?.userId;
    if (!userId || typeof userId !== 'string') {
      throw new McpError(
        ErrorCode.InvalidParams,
    ...
    try {
      ...
      const totalItems = cachedFolders.reduce(
    ...
    try {
      ...
      if (args?.folderIndex !== undefined) {
        ...
        if (index < 0 || index >= cachedFolders.length) {
          throw new McpError(
            ErrorCode.InvalidParams,
            `收藏夹索引 ${index} 无效，共有 ${cachedFolders.length} 个收藏夹`
          );
        }
        const note = await summarizer.summarizeFolder(
          cachedFolders[index]
        );
        notes = [note];
      } else {
